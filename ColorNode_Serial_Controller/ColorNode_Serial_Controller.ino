//*********************************************************************
//* ColorNode String w/Control Software 
//* by Paul Martis
...
#define NODE_NUM 30  //This node number (1-30)
...
#define PATTERN_DELAY 30
#define SEQUENCE_DELAY 1
...
char helpText1[] PROGMEM = 
    ...
    "    option 0: in from black 1: out to black" "\n"
    "4 = Chase (from start bulb to end bulb w/ color)" "\n"
    "    option 0: up, 1: down, 3: fill up, 4: clear down" "\n"
    "    duration # - chase delay between steps (ms)" "\n"
    "5 = Random (from start bulb to end bulb)" "\n"
    "    option # - how many random bulbs at same time" "\n"
    "    duration # - delay between changing bulbs (ms)" "\n"
    ...
    "    duration # - dissolve speed (ms)" "\n"
    "\n"
    "  <m>=mode <d>=duration, <o>=option, <i>=intensity, <n>=node" "\n"
    "  <R>=Red, <G>=Green, <B>=blue, <s>=start bulb, <e>=end bulb" "\n"
    "\n"
    "  <m>,<s>,<e>,<R>,<G>,<B>,<i>,<o>,<d>,<n> l  - light command" "\n"
    ...
    if ('0' <= c && c <= '9')
        ...
        switch (c) {
            ...
            case 'l': //Light Command
                  command.dest = value;
                  command.duration = stack[top-1];
                  command.option = stack[top-2];
                  command.bulb_intensity = stack[top-3];
                  command.bulb_color = lights.color(stack[top-6], stack[top-5], stack[top-4]);
                  command.end_bulb = stack[top-7];
                  command.start_bulb = stack[top-8];
                  command.mode = stack[top-9];
                  pendingOutput = 1;
                  serialCommand = 1;
                break;
            case 'o': // off
...
void loop() {
  ...
  if (pendingOutput) {
    rf12_easySend(&command, sizeof command);
    myCommand.mode = command.mode;
    myCommand.start_bulb = command.start_bulb;
    myCommand.end_bulb = command.end_bulb;
    myCommand.bulb_color = command.bulb_color;
    myCommand.bulb_intensity = command.bulb_intensity;
    myCommand.option = command.option;
    myCommand.duration = command.duration;
    pendingOutput = 0;
  }
  ...
  //Run sequence if no wireless commands were received 
  if (serialCommand == 0) {
    switch (pattern) { //Switch between fills, chases, etc.
      case 0: //Solid Fills
        if (now() - sequenceTime > SEQUENCE_DELAY+4){
          command.mode = 3;
          command.start_bulb = 0;
          command.end_bulb = 49;
          command.bulb_intensity = 204;
          command.option = 0;
          command.duration = 20;
          command.dest = 255;
          pendingOutput = 1;
          switch (sequence) { //Switch Colors
            case 0:
              command.bulb_color = COLOR_WHITE;
              break;
            case 1:
              command.bulb_color = COLOR_RED;
              break;
            case 2:
              command.bulb_color = COLOR_GREEN;
              break;
            case 3:
              command.bulb_color = COLOR_BLUE;
              break;
            case 4:
              command.bulb_color = COLOR_CYAN;
              break;
            case 5:
              command.bulb_color = COLOR_MAGENTA;
              break;
            case 6:
              command.bulb_color = COLOR_YELLOW;
              break;
            case 7:
              command.bulb_color = COLOR_PURPLE;
              break;
            case 8:
              command.bulb_color = COLOR_ORANGE;
              break;
            case 9:
              command.bulb_color = COLOR_WARMWHITE;
              break;
            case 10:
              command.bulb_color = COLOR_WARMWHITE;
              command.option = 1;
        ...
        break;
      case 1: //Basic chasing in then out to black
        if (now() - sequenceTime > SEQUENCE_DELAY){
          command.mode = 4;
          command.start_bulb = 0;
          command.end_bulb = 49;
          command.bulb_intensity = 204;
          ...
          switch (sequence) {
            ...
            case 2:
              ...
              break;
            case 3:
              command.bulb_color = COLOR_BLACK;
              command.bulb_intensity = 0;
              command.option = 3;
              break;
          }
          sequence++;
        ...
        if (sequence > 3)
          sequence = 0;   
        if (now() - patternTime > PATTERN_DELAY){
          pattern++;
        }
        break;
      case 2: //Intermediate Step
          ...
          pattern++;
        break;
      case 3: //Cycle through rest of Stock Plus Patterns
        if (now() - patternTime > PATTERN_DELAY) {
          command.mode = 1;
          command.start_bulb = 0;
          command.end_bulb = 0;
        ...
        break;
      case 4: //Cycle through MEO Rainbows
        if (now() - patternTime > PATTERN_DELAY) {
          command.mode = 1;
          command.start_bulb = 0;
          command.end_bulb = 0;
          command.bulb_color = COLOR_BLACK;
          command.bulb_intensity = sequence;
          command.option = 23;
          command.duration = 0;
          command.dest = 255;
          pendingOutput = 1;
        ...
        break;
      case 5: //Cycle through MEO Sine Waves
        if (now() - patternTime > PATTERN_DELAY) {
          command.mode = 1;
          command.start_bulb = 0;
          command.end_bulb = 0;
        ...
        break;
      case 6: //Cycle through MEO Chasers
        if (now() - patternTime > PATTERN_DELAY) {
          command.mode = 1;
          command.start_bulb = 0;
          command.end_bulb = 0;
    ...
    if ((pattern > 6) && (now() - patternTime > PATTERN_DELAY)){
      pattern = 0;
      patternTime = now();
    }
  }  
